...
最后按代码核对时间：2026-03-12

这份文档按当前仓库里的真实读取点整理，不按旧的全量 Docker 部署路径倒推。

当前有效的部署边界：

- 应用服务使用手动方式启动：`api`、`celery-worker`、`web`、`hot-news`
- 只保留“本地基础依赖”的 Docker 启动路径：`docker-compose-local.yaml` + `.env.local`
- 旧的整套应用 Docker / Compose 启动路径已经移除，不作为当前文档基准
...
| Hot News 服务 | `hot-news/.env.example` | `hot-news/.env` | `dotenv.config()` | 当前仓库已补充样例文件 |
...
- 根目录旧的 `.env.example` 已不再是当前路径的一部分。
...
- `UNION_PAY_API_PREFIX` 虽然不是 `NEXT_PUBLIC_*`，但也被 `next.config.ts` 显式注入了客户端 bundle。
...
| `UNION_PAY_URL_PREFIX` | api, worker | 功能级必填 | 无 | `api/common/dependencies.py`, `celery-worker/common/dependencies.py` | 会员/支付系统后端地址 |
| `WEB_BASE_URL` | api, worker | 推荐配置 | 无 | `api/notification/tool/*.py`, `celery-worker/notification/tool/*.py` | 把相对链接补成完整通知链接 |
...
| `NEXT_PUBLIC_HOST` | 推荐配置 | `http://localhost:3000` | `web/next.config.ts`, `section-publish.tsx` | 分享链接与前端自引用链接 |
| `NEXT_PUBLIC_DEPLOY_HOSTS` | 可选 | 官方域名列表示例 | `web/src/lib/utils.ts`, `account/page.tsx` | 判断是否显示官方订阅能力 |
| `UNION_PAY_API_PREFIX` | 支付/订阅相关功能必填 | 空 | `web/next.config.ts`, `web/src/api/user.ts`, `web/src/api/product.ts`, `web/src/api/order.ts` | 前端访问支付系统的 API 前缀 |
...
| `REDIS_HOST` | `127.0.0.1` | Hot News 自己的 Redis 主机 |
