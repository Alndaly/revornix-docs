...
| `NEXT_PUBLIC_DAILY_HOT_API_PREFIX`       | `http://localhost/api/hot-news-service`          | Daily Hot 聚合接口地址           | `web/src/config/api.ts:3`                      |
...
| 变量              | 默认值      | 说明                          | 主要作用范围                 |
| ----------------- | ----------- | ----------------------------- | ---------------------------- |
| `PORT`            | `6688`      | Daily Hot 服务监听端口        | `hot-news/src/config.ts:36` |
...
- **Docker 部署：**  
  `docker-compose.yaml` 会从根目录 `.env` 读取变量，并传入 `api`、`celery-worker`、`web`、`hot-news` 等服务。确保该文件内容与部署环境一致。
- **敏感信息管理：**  
