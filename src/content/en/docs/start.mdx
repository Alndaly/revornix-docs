...
Please go to the `router/ai.py` file in the API service, find the `ask_ai` function, and modify this code segment according to your actual situation to select an appropriate model.

<Callout>Currently only models from Volcano Engine are supported. Please visit the [Volcano Engine official website](https://www.volcengine.com) to create one.</Callout>
