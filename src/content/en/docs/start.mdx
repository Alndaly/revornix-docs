...
**Note: If your machine is in China mainland, please configure the system dependency mirror source in the `Dockerfile` of the `api` and `celery-worker` directories.**
...
<Callout>
	If you don't have Postgres, Redis, and Milvus installed, you'll need to manually install these services locally and modify the corresponding parameter configurations in the environment variable files.
