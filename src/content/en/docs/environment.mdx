...
| `NEXT_PUBLIC_DAILY_HOT_API_PREFIX`       | `http://localhost/api/hot-news-service`          | Daily Hot aggregation endpoint           | `web/src/config/api.ts:3`                      |
...
| ----------------- | ----------- | --------------------------------------- | ---------------------------- |
| `PORT`            | `6688`      | Listening port for Daily Hot            | `hot-news/src/config.ts:36` |
...
- **Docker deployment:** `docker-compose.yaml` reads variables from the root `.env` and passes them to services such as `api`, `celery-worker`, `web`, and `hot-news`. Ensure the contents match your target environment.
