...
response = client.post(
    url="http://localhost:8001/tp/file/upload",
    headers=headers,
    files=files,
    data=data
)
...
| process_task_trigger_scheduler | string | No       | Schedule for time trigger (cron expression)            |
...
| Parameter    | Type    | Required | Description |
| ------------ | ------- | -------- | ----------- |
| category     | int     | Yes      | Document type, 0: file document, 1: link document, 2: quick note document, 3: audio document |
