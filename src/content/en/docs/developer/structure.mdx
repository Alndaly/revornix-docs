...
- MCP (built-in): Revornix provides both MCP client and server capability. On the client side it supports Stream MCP / Std MCP; on the server side the capability is built into the API under `api/mcp_router/*`, currently grouped mainly into `common`, `document`, and `graph`.
